...
enum state {upper_idle=0,upper_processing};
...
void morse_code_light(char* morse_code){
    ...
    for (int i=0; morse_code[i] !='\n' && morse_code[i] !='\0';i++){
        ...
        if (morse_code[i] == '.') {
            set_led_status(true);
            vTaskDelay(pdMS_TO_TICKS(DOT LENGTH));//amount of ticks to indicate its a dot
            set_led_status(false);

            vTaskDelay(pdMS_TO_TICKS(TIME BETWEEN SYMBOLS))// so they arent actuated right after each other
        } 
        else if (morse_code[i] == '-') { 
            set_led_status(true);
            vTaskDelay(pdMS_TO_TICKS(DASH LENGTH));//amount of ticks to indicate its a dash
            set_led_status(false);
            ...
            if (morse_code[i+1] == ' ') {
                vTaskDelay(pdMS_TO_TICKS(NEXT WORD));//amount of ticks to indicate space between two words
                
            else{vTaskDelay(pdMS_TO_TICKS(SPACE LENGTH));}//amount of ticks to indicate its a space
        }
